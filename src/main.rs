...
#[derive(Clone, Copy)]
struct IoEntry { total: u64, dr: u64, dw: u64, comm: [u8; COMM_LEN], cl: u8 }

struct Top5 { e: [TopEntry; TOP_N], n: usize }
struct IoTop5 { e: [IoEntry; TOP_N], n: usize }

const EMPTY_TE: TopEntry = TopEntry { val: 0, comm: [0; COMM_LEN], cl: 0 };
const EMPTY_IE: IoEntry = IoEntry { total: 0, dr: 0, dw: 0, comm: [0; COMM_LEN], cl: 0 };

impl Top5 {
    fn new() -> Self { Self { e: [EMPTY_TE; TOP_N], n: 0 } }
    fn insert(&mut self, val: u64, comm: &[u8]) {
        let mut c = [0u8; COMM_LEN];
        let l = comm.len().min(COMM_LEN);
        c[..l].copy_from_slice(&comm[..l]);
        let e = TopEntry { val, comm: c, cl: l as u8 };
        if self.n < TOP_N { self.e[self.n] = e; self.n += 1; }
        else { let mi = self.min_idx(); if val > self.e[mi].val { self.e[mi] = e; } }
    }
    fn min_idx(&self) -> usize {
        let (mut mi, mut mv) = (0, self.e[0].val);
        let mut i = 1; while i < self.n { if self.e[i].val < mv { mi = i; mv = self.e[i].val; } i += 1; }
        mi
    }
...
impl IoTop5 {
    fn new() -> Self { Self { e: [EMPTY_IE; TOP_N], n: 0 } }
    fn insert(&mut self, total: u64, dr: u64, dw: u64, comm: &[u8]) {
        let mut c = [0u8; COMM_LEN];
        let l = comm.len().min(COMM_LEN);
        c[..l].copy_from_slice(&comm[..l]);
        let e = IoEntry { total, dr, dw, comm: c, cl: l as u8 };
        if self.n < TOP_N { self.e[self.n] = e; self.n += 1; }
        else { let mi = self.min_idx(); if total > self.e[mi].total { self.e[mi] = e; } }
    }
    fn min_idx(&self) -> usize {
        let (mut mi, mut mv) = (0, self.e[0].total);
        let mut i = 1; while i < self.n { if self.e[i].total < mv { mi = i; mv = self.e[i].total; } i += 1; }
        mi
    }
...
fn pread_raw(f: &fs::File, buf: &mut [u8]) -> usize {
    let n = unsafe { libc::pread(f.as_raw_fd(), buf.as_mut_ptr() as *mut libc::c_void, buf.len(), 0) };
...
impl PidStats {
    fn with_capacity(n: usize) -> Self { Self { entries: Vec::with_capacity(n) } }
    fn clear(&mut self) { self.entries.clear(); }
    fn push(&mut self, pid: u32, st: BpfPidStats) { self.entries.push((pid, st)); }
    fn sort(&mut self) { self.entries.sort_unstable_by_key(|e| e.0); }
    fn get(&self, pid: u32) -> Option<&BpfPidStats> {
        self.entries.binary_search_by_key(&pid, |e| e.0).ok().map(|i| &self.entries[i].1)
    }
}
...
#[repr(C)]
struct BpfAttrMapCreate { map_type: u32, key_size: u32, value_size: u32, max_entries: u32 }

#[derive(Clone, Copy)]
struct BpfMapDef {
    name: &'static str,
    map_type: u32, key_size: u32, value_size: u32, max_entries: u32,
    fd: RawFd,
}
...
#[repr(C)]
struct BpfAttrBatch {
    in_batch: u64, out_batch: u64,
    keys: u64, values: u64,
    count: u32, map_fd: u32,
    elem_flags: u64, flags: u64,
}
...
fn bpf_map_create(mt: u32, ks: u32, vs: u32, me: u32) -> Option<RawFd> {
    let a = BpfAttrMapCreate { map_type: mt, key_size: ks, value_size: vs, max_entries: me };
...
fn bpf_map_get_next_key(fd: RawFd, key: Option<&[u8]>, next: &mut [u8]) -> bool {
    #[repr(C)] struct A { fd: u32, _p: u32, key: u64, next: u64 }
...
fn bpf_prog_load(pt: u32, insns: &[u8], lic: &[u8], log: &mut [u8]) -> Option<RawFd> {
    ...
    struct A {
        ...
        kern_version: u32, prog_flags: u32, _pad: [u64; 16],
    }
    let a = A {
        prog_type: pt, insn_cnt: (insns.len() / BPF_INSN_SIZE) as u32,
        insns: insns.as_ptr() as u64, license: lic.as_ptr() as u64,
        log_level: if log.is_empty() { 0 } else { 1 },
        log_size: log.len() as u32, log_buf: log.as_mut_ptr() as u64,
...
fn perf_event_open_tracepoint(tp_id: u64, cpu: i32) -> Option<RawFd> {
    ...
    a.config = tp_id;
    let fd = unsafe { libc::syscall(libc::SYS_perf_event_open, &a as *const _, -1i32, cpu, -1i32, 0u64) };
    if fd < 0 { None } else { Some(fd as RawFd) }
}
...
impl BpfLoader {
    fn load(obj_path: &str) -> Option<Self> {
        ...
        let mut maps = [
            BpfMapDef { name: "stats", map_type: BPF_MAP_TYPE_HASH,
                key_size: 4, value_size: std::mem::size_of::<BpfPidStats>() as u32,
                max_entries: MAX_PIDS as u32, fd: -1 },
            BpfMapDef { name: "sys", map_type: BPF_MAP_TYPE_ARRAY,
        ...
        if let Some(mi) = maps_shndx {
            for (si, sym) in elf.syms.iter().enumerate() {
                if sym.st_shndx == mi {
                    ...
                    for m in &maps { if m.name == name { sym_to_fd.insert(si, m.fd); } }
                }
            }
        }
        ...
        for (shndx, sec_name) in &prog_sections {
            ...
            let fd = match bpf_prog_load(BPF_PROG_TYPE_TRACEPOINT, &insns, license, &mut log) {
                ...
                None => {
                    ...
                    if !ls.is_empty() { eprintln!("bpf: prog load failed for {sec_name}:\n{ls}"); }
                    else { eprintln!("bpf: prog load failed for {sec_name}: {}", io::Error::last_os_error()); }
            ...
            let tp_id = match tracepoint_id(cat, tp) {
                Some(id) => id,
                None => { eprintln!("bpf: tracepoint {cat}/{tp} not found"); continue; }
            };
            ...
            for cpu in 0..ncpu {
                ...
                if cpu == 0 {
                    ...
                    if r < 0 {
                        eprintln!("bpf: attach failed for {sec_name}: {}", io::Error::last_os_error());
    ...
    fn read_stats(&mut self, out: &mut PidStats) {
        out.clear();
        if self.use_batch {
            if self.read_batch(out) { out.sort(); return; }
    ...
    fn read_batch(&mut self, out: &mut PidStats) -> bool {
        ...
        loop {
            ...
            if r < 0 {
                ...
                if e.raw_os_error() == Some(libc::ENOENT) { break; }
                if total == 0 { return false; }
                break;
            }
            first = false;
        }
        for i in 0..total { out.push(self.bk[i], self.bv[i]); }
        true
    }
...
fn take_sample(
    ...
    for &(pid, ref st) in &cur.entries {
        ...
        if rss >= MIN_MEM_BYTES { top_mem.insert(rss, &st.comm[..cl]); }
        ...
        let dt = drb + dwb;
        if dt >= min_io { top_io.insert(dt, drb, dwb, &st.comm[..cl]); }
    }
...
fn emit(prev: Option<&Sample>, cur: &mut Sample, dur: Duration, tt: &mut String, json: &mut String, text_buf: &mut String) {
    ...
    // Build tooltip
    tt.clear();
    let _ = write!(tt, "Load: {:.2} {:.2} {:.2} ({} cores)", cur.load[0], cur.load[1], cur.load[2], cur.cores);
    ...
    match prev {
        Some(p) => {
            ...
            if dt_ms > 0 {
                ...
                let _ = write!(tt, "\niGPU: {busy:.0}% @ {}/{} MHz", cur.gpu_freq, cur.gpu_max);
            } else {
...
fn main() {
    ...
    emit(None, &mut s, Duration::ZERO, &mut tt, &mut json, &mut text_buf);
